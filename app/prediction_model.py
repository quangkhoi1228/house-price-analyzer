...
def get_answer_bot(API_KEY, user_question):
    ...
    llm = OpenAI(temperature=0.7, openai_api_key=API_KEY, max_tokens=-1)
...
def prediction_model_pipeline(DF, API_KEY, GPT_MODEL, QUESTION):
    ...
    if not st.session_state.target_selected:
        ...
        with st.spinner("AI is analyzing the data..."):
            ...
            st.session_state.target_Y = decide_target_attribute_classi(
                attributes_for_target, types_info_for_target, head_info_for_target, GPT_MODEL, API_KEY, THE_PROMPT
            )
    ...
    if st.session_state.target_selected:
        ...
        if "encoded_df" not in st.session_state:
            if not st.session_state.all_numeric:
                ...
                st.session_state.json_file = convert_question_to_DF(
                    attributes_for_target, types_info_for_target, head_info_for_target, GPT_MODEL, API_KEY, THE_PROMPT
                )
...
def display_results(X_train, X_test, Y_train, Y_test):
    ...
    with model_col1:
        ...
        result1 = {
            ...
            "confusion_matrix": get_confusion_metrix(
